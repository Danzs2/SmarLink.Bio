...
<head>
    ...
    <style>
        ...
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; color: #334155; }
...
<body class="bg-dots antialiased" 
      x-data="{ 
          ...
          // PENTING: Perbaikan agar URL gambar tidak error jika ada backslash dari Windows
          ...
          newCustom: { title: '', url: '', is_private: 0, privacy_mode: 'public', link_password: '', allowed_emails: '' },
          ...
          // PENGIRIMAN DATA YANG SUDAH DIPERBAIKI (Tarik file pakai ID DOM yang BENAR)
          saveAppearance() {
              ...
              // Ambil file langsung dari DOM HTML
              ...
              .then(data => {
                  if(data.success) {
                      ...
                      // Bersihkan value agar tidak error kalau upload gambar yang sama berulang kali
          ...
          openEdit(link) {
              this.editData = { ...link, privacy_mode: link.is_private === 1 ? (link.link_password ? 'password' : 'email') : 'public' };
          ...
          copyToClipboard() {
              ...
              navigator.clipboard.writeText(text).then(() => { alert('Tautan berhasil disalin ke clipboard!\n\n' + text); });
          }
      }">
    ...
    <div class="flex h-screen overflow-hidden pb-16 md:pb-0 z-10 relative">

        <aside class="w-64 bg-white/90 backdrop-blur-xl border-r border-gray-200 flex flex-col hidden md:flex shrink-0 shadow-lg shadow-gray-200/50">
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden relative bg-transparent">
            ...
            @if(session('success'))
            <div class="absolute top-4 left-1/2 transform -translate-x-1/2 z-50 bg-emerald-500 text-white px-6 py-3 rounded-full font-bold shadow-lg flex items-center gap-2">
                <i class="fa-solid fa-circle-check"></i> {{ session('success') }}
            </div>
            @endif

            <header class="h-20 flex items-center justify-between px-6 lg:px-12 z-10 shrink-0 bg-white/60 backdrop-blur-xl border-b border-gray-200 shadow-sm">
                <div class="hidden sm:block">
                    <p class="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">Tautan Publik SmartLink-mu</p>
                    <a href="{{ url('/@'.Auth::user()->username) }}" target="_blank" class="text-sm font-semibold text-indigo-600 hover:underline">smartlink.com/{{ '@' . (Auth::user()->username ?? 'username') }}</a>
                </div>
                <h1 class="text-xl font-bold sm:hidden text-gray-900" x-text="tab === 'links' ? 'Tautan' : (tab === 'appearance' ? 'Desain' : (tab === 'analytics' ? 'Analitik' : 'Pengaturan'))"></h1>
                <div class="flex items-center gap-3">
                    <button @click="copyToClipboard()" class="bg-gray-900 text-white px-6 py-2.5 rounded-full text-sm font-bold hover:bg-black transition shadow-lg shadow-gray-900/20 flex items-center gap-2 transform active:scale-95">
                        <i class="fa-solid fa-share-nodes"></i> Bagikan
                    </button>
                </div>
            </header>

            <div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:px-12 lg:pb-24 scroll-smooth scrollbar-hide">
                <div class="max-w-[640px] mx-auto">

                    <div x-show="tab === 'links'" x-transition.opacity.duration.300ms>
                        
                        <div x-show="addMode === ''" class="flex flex-col sm:flex-row gap-4 mb-8">
                            <button @click="addMode = 'custom'" class="flex-1 bg-indigo-600 text-white rounded-2xl py-4 font-bold text-sm hover:bg-indigo-700 transition shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2 transform active:scale-95">
                                <i class="fa-solid fa-plus"></i> Tambah Kustom
                            </button>
                            <button @click="addMode = 'social'" class="flex-1 bg-white text-gray-900 border border-gray-200 rounded-2xl py-4 font-bold text-sm hover:bg-gray-50 transition shadow-sm flex items-center justify-center gap-2 transform active:scale-95">
                                <i class="fa-solid fa-icons text-blue-500"></i> Tambah Ikon Sosial
                            </button>
                        </div>
                        ...
                        <div x-show="addMode === 'custom'" x-collapse class="card-glass rounded-[24px] shadow-soft mb-8 overflow-hidden">
                            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-white/50">
                                <h3 class="font-bold text-gray-900 flex items-center gap-2"><i class="fa-solid fa-link text-indigo-600"></i> Tautan Kustom Baru</h3>
                            ...
                            <form action="{{ route('links.store') }}" method="POST" class="p-6 space-y-6">
                                @csrf
                                ...
                                <div class="p-4 rounded-xl border border-gray-200 bg-white/50">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <label class="cursor-pointer">
                                            ...
                                            <div class="px-4 py-2 border-2 border-gray-200 bg-white rounded-full text-sm font-semibold text-gray-500 peer-checked:border-indigo-600 peer-checked:text-indigo-600 peer-checked:bg-indigo-50 transition flex items-center gap-2">
                                        ...
                                        <label class="cursor-pointer">
                                            ...
                                            <div class="px-4 py-2 border-2 border-gray-200 bg-white rounded-full text-sm font-semibold text-gray-500 peer-checked:border-orange-500 peer-checked:text-orange-600 peer-checked:bg-orange-50 transition flex items-center gap-2">
                                                <i class="fa-solid fa-key"></i> Sandi/PIN
                                            </div>
                                        </label>
                                        <label class="cursor-pointer">
                                            <input type="radio" name="privacy_mode" value="email" x-model="newCustom.privacy_mode" @change="updatePrivacy()" class="peer sr-only">
                                            <div class="px-4 py-2 border-2 border-gray-200 bg-white rounded-full text-sm font-semibold text-gray-500 peer-checked:border-blue-500 peer-checked:text-blue-600 peer-checked:bg-blue-50 transition flex items-center gap-2">
                                                <i class="fa-solid fa-envelope"></i> Spesifik Email
                                            </div>
                                        </label>
                                    </div>
                                    ...
                                    <div x-show="newCustom.privacy_mode === 'email'" x-collapse class="mt-3">
                                        <textarea name="allowed_emails" x-model="newCustom.allowed_emails" rows="2" placeholder="Masukkan daftar email (pisahkan dengan koma)" class="w-full bg-white border border-blue-200 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-none"></textarea>
                                    </div>
                                </div>

                                <div class="flex justify-end pt-2">
                                    <button type="submit" class="bg-indigo-600 text-white px-8 py-3 rounded-full font-bold hover:bg-indigo-700 transition shadow-md w-full sm:w-auto">Simpan Kustom</button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div x-show="addMode === 'social'" x-collapse class="card-glass rounded-[24px] shadow-soft mb-8 overflow-hidden">
                            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-white/50">
                                <h3 class="font-bold text-gray-900 flex items-center gap-2"><i class="fa-solid fa-icons text-blue-500"></i> Tambah Ikon Sosial</h3>
                                <button @click="addMode = ''" class="w-8 h-8 rounded-full hover:bg-gray-200 flex items-center justify-center text-gray-500 transition"><i class="fa-solid fa-xmark"></i></button>
                            </div>
                            
                            <form action="{{ route('links.store') }}" method="POST" class="p-6 space-y-5">
                                @csrf
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-4 sm:grid-cols-6 gap-3 max-h-[160px] overflow-y-auto pr-2">
                                        <template x-for="plat in platforms" :key="plat.id">
                                            <button type="button" @click="newSocial.platform = plat.id" 
                                                    ...
                                                    class="flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition aspect-square">
                                ...
                                <div x-show="newSocial.platform" x-collapse>
                                    ...
                                    <div class="flex justify-end pt-4">
                                        <button type="submit" class="bg-gray-900 text-white px-8 py-3 rounded-full font-bold hover:bg-black transition shadow-md w-full sm:w-auto">Simpan Sosial</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="space-y-4">
                            <template x-for="link in links" :key="link.id">
                                <div class="card-glass rounded-[24px] p-5 shadow-soft hover:shadow-hover transition-shadow flex flex-col sm:flex-row gap-4 relative group mb-4">
                                    ...
                                    <div class="flex-1 min-w-0 pr-12 sm:pr-0">
                                        <div class="flex items-center gap-2 mb-1">
                                            <div class="w-6 h-6 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center shrink-0">
                                                <i :class="link.type === 'social' ? getPlatformIcon(link.platform) : 'fa-solid fa-link'" class="text-xs text-indigo-600"></i>
                                            </div>
                                            <h4 class="font-bold text-gray-900 text-base truncate" x-text="link.title"></h4>
                                            <button @click="openEdit(link)" class="text-gray-400 hover:text-indigo-600 transition">
                                                <i class="fa-solid fa-pen text-xs ml-1"></i>
                                            </button>
                                        </div>

                                        <div class="flex items-center gap-2 pl-8">
                                            <a :href="link.url" target="_blank" class="text-gray-500 text-sm hover:underline truncate" x-text="link.url"></a>
                                        </div>
                                        ...
                                        <div class="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100">
                                            ...
                                            <span x-show="link.type === 'custom'" class="flex items-center gap-1 text-[11px] font-bold" :class="link.is_private === 0 ? 'text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded' : 'text-rose-600 bg-rose-50 px-2 py-0.5 rounded'">
                                            ...
                                            <span class="flex items-center gap-1.5 text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded ml-2">
                                    ...
                                    <div class="absolute top-5 right-5 sm:relative sm:top-0 sm:right-0 flex items-start">
                                        <button @click="toggleLink(link.id)" :class="link.is_active === 1 ? 'bg-emerald-500' : 'bg-gray-200'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none">
                                            <span :class="link.is_active === 1 ? 'translate-x-6' : 'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow-sm"></span>
                                        </button>
                                    </div>
                                </div>
                            </template>
                            ...
                            <div x-show="links.length === 0" class="text-center py-10">
                                ...
                                <p class="text-gray-500 font-semibold">Tautan kamu masih kosong.</p>
                            </div>
                        </div>
                    </div>

                    <div x-show="tab === 'appearance'" x-cloak x-transition.opacity.duration.300ms class="space-y-8">
                        <div class="card-glass rounded-[24px] p-6 sm:p-8 shadow-soft">
                            <h3 class="font-bold text-gray-900 text-lg mb-6">Profil</h3>
                            <div class="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                                <div class="relative shrink-0 group">
                                    <img :src="profileImagePreview || 'https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name ?? 'User') }}&background=000&color=fff'" alt="Profile" class="w-24 h-24 rounded-full border border-gray-200 object-cover shadow-sm group-hover:opacity-70 transition">
                        ...
                        <div class="card-glass rounded-[24px] p-6 sm:p-8 shadow-soft">
                            <h3 class="font-bold text-gray-900 text-lg mb-4">Latar Belakang Profil</h3>
                            <div class="flex gap-4 mb-6 bg-gray-50 p-2 rounded-2xl border border-gray-200">
                                <button @click="userProfile.bg_type = 'solid'" :class="userProfile.bg_type === 'solid' ? 'bg-white shadow-sm font-bold text-gray-900' : 'text-gray-500 hover:text-gray-700'" class="flex-1 py-2.5 rounded-xl text-sm transition">Solid</button>
                                <button @click="userProfile.bg_type = 'gradient'" :class="userProfile.bg_type === 'gradient' ? 'bg-white shadow-sm font-bold text-gray-900' : 'text-gray-500 hover:text-gray-700'" class="flex-1 py-2.5 rounded-xl text-sm transition">Gradient</button>
                                <button @click="userProfile.bg_type = 'image'" :class="userProfile.bg_type === 'image' ? 'bg-white shadow-sm font-bold text-gray-900' : 'text-gray-500 hover:text-gray-700'" class="flex-1 py-2.5 rounded-xl text-sm transition">Gambar</button>
                            </div>
                            ...
                            <div x-show="userProfile.bg_type === 'image'" x-collapse class="mt-4">
                                <label class="block w-full border-2 border-dashed border-gray-300 bg-white rounded-xl p-6 text-center cursor-pointer hover:bg-gray-50 hover:border-indigo-400 transition">
                                    <i class="fa-solid fa-cloud-arrow-up text-indigo-400 text-3xl mb-2"></i>
                                    <p class="text-sm text-gray-600 font-bold">Klik untuk unggah gambar latar</p>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase">Maksimal 2MB (JPG/PNG)</p>
                        ...
                        <div class="card-glass rounded-[24px] p-6 sm:p-8 shadow-soft">
                            <h3 class="font-bold text-gray-900 text-lg mb-4">Posisi Ikon Sosial</h3>
                            <div class="flex gap-4">
                                <button @click="userProfile.social_position = 'top'" :class="userProfile.social_position === 'top' ? 'ring-2 ring-indigo-500 bg-indigo-50 border-transparent' : 'border-gray-200 bg-white hover:bg-gray-50'" class="flex-1 py-4 border-2 rounded-xl text-sm font-bold text-gray-700 transition flex flex-col items-center gap-2">
                                ...
                                <button @click="userProfile.social_position = 'bottom'" :class="userProfile.social_position === 'bottom' ? 'ring-2 ring-indigo-500 bg-indigo-50 border-transparent' : 'border-gray-200 bg-white hover:bg-gray-50'" class="flex-1 py-4 border-2 rounded-xl text-sm font-bold text-gray-700 transition flex flex-col items-center gap-2">
                                    <div class="w-12 h-1 bg-gray-300 rounded-full mb-1"></div>
                        ...
                        <div class="card-glass rounded-[24px] p-6 sm:p-8 shadow-soft">
                            <h3 class="font-bold text-gray-900 text-lg mb-4">Pengaturan Tombol Kustom</h3>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Bentuk Tombol</p>
                            <div class="grid grid-cols-3 gap-3 mb-6">
                                <button @click="userProfile.button_corner_style = 'square'" :class="userProfile.button_corner_style === 'square' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-16 h-4 bg-gray-800"></div> 
                                </button>
                                <button @click="userProfile.button_corner_style = 'rounded'" :class="userProfile.button_corner_style === 'rounded' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-16 h-4 bg-gray-800 rounded-md"></div> 
                                </button>
                                <button @click="userProfile.button_corner_style = 'capsule'" :class="userProfile.button_corner_style === 'capsule' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-16 h-4 bg-gray-800 rounded-full"></div> 
                                </button>
                            </div>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Gaya Tampilan</p>
                            <div class="grid grid-cols-3 gap-3 mb-6">
                                <button @click="userProfile.button_display_style = 'fill'" :class="userProfile.button_display_style === 'fill' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-20 h-6 bg-gray-900 rounded-md"></div> 
                                </button>
                                <button @click="userProfile.button_display_style = 'outline'" :class="userProfile.button_display_style === 'outline' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-20 h-6 border-2 border-gray-900 rounded-md"></div> 
                                </button>
                                <button @click="userProfile.button_display_style = 'shadow'" :class="userProfile.button_display_style === 'shadow' ? 'ring-2 ring-black bg-gray-100' : 'border-gray-200 bg-white hover:bg-gray-50'" class="py-5 border-2 rounded-xl flex justify-center transition">
                                    <div class="w-20 h-6 bg-white shadow-md border border-gray-200 rounded-md"></div> 
                                </button>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Warna Tombol</p>
                                    <div class="flex items-center gap-3">
                                        <input type="color" x-model="userProfile.button_color">
                                        <span class="font-mono text-sm font-bold text-gray-800 uppercase" x-text="userProfile.button_color"></span>
                                    </div>
                                </div>
                                <div class="bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Warna Teks</p>
                                    <div class="flex items-center gap-3">
                                        <input type="color" x-model="userProfile.text_color">
                                        <span class="font-mono text-sm font-bold text-gray-800 uppercase" x-text="userProfile.text_color"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex justify-end">
                            <button @click="saveAppearance()" class="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all transform active:scale-95">
                                <i class="fa-solid fa-floppy-disk mr-2"></i> Simpan Perubahan Desain
                            </button>
                        </div>
                    </div>
                    ...
                    <div x-show="tab === 'analytics'" x-cloak x-transition.opacity.duration.300ms class="space-y-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="bg-gray-900 rounded-[24px] p-6 shadow-xl text-white relative overflow-hidden">
                                <div class="absolute -right-4 -top-4 w-24 h-24 bg-white/10 rounded-full"></div>
                                <p class="text-gray-400 font-medium mb-1">Kunjungan Profil</p>
                                <h2 class="text-5xl font-black mb-2">{{ Auth::user()->visits ?? 0 }}</h2>
                            </div>
                            <div class="card-glass rounded-[24px] p-6 shadow-soft">
                                <p class="text-gray-500 font-semibold mb-1">Total Klik Semua Tautan</p>
                                <h2 class="text-5xl font-black text-indigo-600 mb-2" x-text="totalClicks"></h2>
                            </div>
                        </div>
                    </div>

                    <div x-show="tab === 'settings'" x-cloak x-transition.opacity.duration.300ms class="space-y-6">
                        <div class="card-glass rounded-[24px] p-6 sm:p-8 shadow-soft border-t-4 border-t-rose-500">
    ...
    {{ Auth::user()->violation_count ?? '0' }}
        ...
        <aside class="w-[360px] xl:w-[400px] border-l border-white/40 hidden lg:flex items-center justify-center p-8 z-10 shrink-0 bg-white/20 backdrop-blur-sm relative">
            <div class="relative w-[280px] h-[580px] border-[10px] border-gray-900 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col z-10 ring-1 ring-gray-200 bg-white">
                ...
                <div class="flex-1 w-full h-full overflow-y-auto flex flex-col items-center pt-14 pb-10 scrollbar-hide transition-all duration-300 relative" 
                     :style="bgStyle">
        ...
        <nav class="md:hidden fixed bottom-0 inset-x-0 bg-white/90 backdrop-blur-md border-t border-gray-200 flex justify-around items-center h-16 z-50">
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" x-transition.opacity>
        <div @click.away="showEditModal = false" class="bg-white rounded-[24px] w-full max-w-lg shadow-2xl overflow-hidden shadow-indigo-900/20">
            ...
            <form :action="'{{ url('/links') }}/' + editData.id" method="POST" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div class="relative">
                ...
                <div x-show="editData.type === 'custom'" class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="privacy_mode" value="email" x-model="editData.privacy_mode" class="peer sr-only">
                            <div class="px-4 py-2 border-2 border-gray-200 bg-white rounded-full text-sm font-semibold text-gray-500 peer-checked:border-blue-500 peer-checked:text-blue-600 peer-checked:bg-blue-50 transition flex items-center gap-2">
                    ...
                    <div x-show="editData.privacy_mode === 'password'" class="mt-3">
                        <input type="text" name="link_password" x-model="editData.link_password" placeholder="Masukkan kata sandi..." class="w-full bg-white border border-orange-200 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-orange-500">
                    </div>
                    <div x-show="editData.privacy_mode === 'email'" class="mt-3">
                        <textarea name="allowed_emails" x-model="editData.allowed_emails" rows="2" placeholder="Daftar email (pisahkan dengan koma)" class="w-full bg-white border border-blue-200 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
                    </div>
                </div>

                <div class="flex justify-end pt-4 border-t border-gray-100">
                    <button type="submit" class="bg-gray-900 text-white px-8 py-3 rounded-full font-bold hover:bg-black transition shadow-md w-full sm:w-auto">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
